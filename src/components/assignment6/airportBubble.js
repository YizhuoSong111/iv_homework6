...
function AirportBubble(props){
    ...
    // 2. Sort the cities ascendingly by the d.Count
    cities = cities.sort((a, b) => a.Count - b.Count);

    // 3. Define a scale for the radius of bubbles
    const radiusScale = scaleLinear()
      .domain([min(cities, (d) => d.Count), max(cities, (d) => d.Count)])
      .range([2, width * 0.15]);

    // 4. Run the force simulation
    forceSimulation(cities)
      ...
      .force(
        "collide",
        forceCollide((d) => radiusScale(d.Count)).strength(1)
      )
      .tick(200);

    // 5. Return the circles
    // Highlight the top 5 hubs
    const top5Cities = cities.slice(-5);
      ...
      <g>
        {cities.map((d, idx) => (
          <circle
            ...
            fill={top5Cities.includes(d) ? "#ADD8E6" : "#2a5599"}
            stroke={"black"}
            strokeWidth={"2"}
          />
        ))}
        {top5Cities.map((d, idx) => (
          <text
            key={`text-${idx}`}
            x={d.x}
            y={d.y}
